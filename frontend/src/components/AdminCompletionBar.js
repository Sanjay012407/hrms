...
export default function AdminCompletionBar() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (!contentType.includes('application/json')) {
          ...
          throw new Error(`Unexpected response: ${contentType} ${text.slice(0, 120)}...`);
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 cursor-pointer" onClick={() => navigate('/dashboard/admin-details')}>
